...
class AdvertCommand(BaseCommand):
    ...
    def can_execute(self, message: MeshMessage) -> bool:
        ...
        # Additional check for bot's last advert time (legacy support). This
        # follows the same global switch as all other command cooldowns.
        if (
            self.command_cooldowns_enabled()
            and hasattr(self.bot, 'last_advert_time')
            and self.bot.last_advert_time
        ):
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.bot, 'last_advert_time')
                and self.bot.last_advert_time
                and (current_time - self.bot.last_advert_time) < 3600
            ):
