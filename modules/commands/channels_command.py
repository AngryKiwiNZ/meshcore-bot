...
class ChannelsCommand(BaseCommand):
    ...
    def __init__(self, bot):
        ...
        self.menu_timeout_seconds = self.get_config_value(
            'Channels_Command', 'menu_timeout_seconds', fallback=600, value_type='int'
        )
        self._active_menus = {}

    MENU_CHOICES = ('a', 'b', 'c', 'd', 'e')
    EXIT_CHOICES = {'x', 'exit', 'cancel'}

    def _menu_key(self, message: MeshMessage) -> str:
        return message.sender_pubkey or message.sender_id or ''

    def _menu_is_active(self, message: MeshMessage) -> bool:
        cutoff = time.time() - self.menu_timeout_seconds
        self._active_menus = {
            key: started for key, started in self._active_menus.items() if started >= cutoff
        }
        return self._menu_key(message) in self._active_menus

    def _activate_menu(self, message: MeshMessage) -> None:
        key = self._menu_key(message)
        if key:
            self._active_menus[key] = time.time()
    ...
    def _general_channels(self):
        return [
            (name, description)
            for name, description in self._load_channels_from_config().items()
        ]

    def _menu_prompt(self) -> str:
        entries = self._general_channels()[:len(self.MENU_CHOICES)]
        choices = ', '.join(
            f'{letter.upper()} {channel}'
            for letter, (channel, _) in zip(self.MENU_CHOICES, entries)
        )
        return f'Public channels: {choices}. Reply A-E for details or X to exit.'
    ...
    def matches_keyword(self, message: MeshMessage) -> bool:
        ...
        """
        normalized = self._normalize_content(message)
        if self._menu_is_active(message) and (
            normalized in self.MENU_CHOICES or normalized in self.EXIT_CHOICES
        ):
            return True
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            normalized = self._normalize_content(message)
            if self._menu_is_active(message) and normalized in self.EXIT_CHOICES:
                self._active_menus.pop(self._menu_key(message), None)
                return await self.send_response(
                    message, 'Exited channels menu.', skip_user_rate_limit=True
                )

            if self._menu_is_active(message) and normalized in self.MENU_CHOICES:
                index = self.MENU_CHOICES.index(normalized)
                channels = self._general_channels()
                if index < len(channels):
                    channel, description = channels[index]
                    self._activate_menu(message)
                    return await self.send_response(
                        message,
                        f'{channel}: {description} Subscribe by adding {channel} to your MeshCore channels.',
            ...
            if content.lower() in {'channels', 'channel', 'channels list', 'channel list'}:
                if not self._general_channels():
                    await self.send_response(message, self.translate('commands.channels.no_channels_configured'))
                    return True
                self._activate_menu(message)
                return await self.send_response(
                    message, self._menu_prompt(), skip_user_rate_limit=True
                )
