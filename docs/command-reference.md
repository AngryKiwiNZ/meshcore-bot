...
Show a multipart quick-start guide for commonly used commands, or get detailed help for a specific command.

**Usage:**
- `help` - Explain common commands including ping, cmd, advert, sun, airplanes, stats, and satpass
...
**Response:** Sends a numbered practical guide for bare `help`, or detailed information for `help <command>`. Use `cmd` for the compact command list.
...
Show a small repeater-build helper menu with the common CLI commands needed during setup.
...
**Menu options:**
- `1` - Show the 2 byte hop CLI command
- `2` - Show the power savings CLI command for ESP32 boards
- `3` - Show the repeater-mode disable command for setup use
- `4` - Show the clock reset and sync CLI commands
- `5` - Exit the build menu
...
**Response:** Opens a short-lived submenu for the sender only. After `build`, the bot returns the requested setup command when you reply with `1`, `2`, `3`, or `4`, and closes the menu with `5`.
...
Get weather information using MetService for New Zealand locations and Open-Meteo elsewhere.
...
**Default location behavior:** If no location is provided and no companion location is available, the bot uses `default_weather_location` when configured; otherwise it asks for a location.

**Unknown location behavior:** If the requested location cannot be resolved, the bot retries using `default_weather_location` so weather lookups still return a useful result.

---

### `gwx <location>`

Get global weather information for any location worldwide using MetService for New Zealand locations and Open-Meteo elsewhere.
...
gwxa <location>
```
If no location is provided, it uses companion location when available; otherwise it falls back to `default_weather_location`. If the requested location cannot be resolved, it retries with `default_weather_location`.
...
airplanes <place name> [radius]
airplanes <radius|radius=N>
...
**Location Options:**
- No location: Uses companion location, otherwise bot location, then an explicitly configured default location
- `here`: Uses bot location from config
- `<lat>,<lon>`: Uses specified coordinates (e.g., `47.6,-122.3`)
- `<place name>`: Geocodes a city or place (e.g., `Example City`)

**Filter Options:**
- `radius=<nm>` - Search radius in nautical miles (default: 25, max: 250)
- `<number>` or `<number>nm` - Radius shorthand (e.g., `100` or `100nm`)
...
airplanes
airplanes here
airplanes Example City
airplanes Example City 100nm
airplanes 100
...
**Note:** Uses companion location from database if available, otherwise falls back through bot location and explicitly configured defaults. The API is rate-limited to 1 request per second.
...
Get MetService CAP alerts, optionally filtered by locally configured text.
...
**Response:**
- Returns alert descriptions from `https://alerts.metservice.com/cap/rss`; set `metservice_location_filter` locally to limit them
- If no matches: reports the number of current alerts fetched for troubleshooting
...
List and play persistent games on the Bot radio.

**Usage:**
- `games` - Show enabled games
- `game lemonade` or `lemonade` - Start Lemonade Stand
- `game blackjack` or `blackjack` - Start Blackjack
- `game mastermind` or `mastermind` - Start Mastermind
- `game status` - Repeat the current game prompt
- `game stop` - End the current game
- `game top <game>` - Show a leaderboard

Game replies are limited to two compact DM messages. One active game is stored per
contact in SQLite, survives bot restarts, and expires after seven inactive days by
default. Existing deterministic commands such as `wx` and `ping` remain available
during a game.
...
- **`wx`** - Uses companion location when available, otherwise `default_weather_location`
- **`gwx`** - Uses companion location when available, otherwise `default_weather_location`
...
- `wx` and `gwx` use MetService for New Zealand forecasts when possible
- Both fall back to Open-Meteo for international locations
- If a requested weather location cannot be resolved, the bot retries with `Weather.default_weather_location` when it is configured
