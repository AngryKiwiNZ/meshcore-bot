...
class BaseCommand(ABC):
    ...
    def get_queue_threshold_seconds(self) -> float:
        ...
        """
        if not self.command_cooldowns_enabled():
            return 0.0
    ...
    def command_cooldowns_enabled(self) -> bool:
        """Return whether user-facing command cooldowns are enabled globally.

        The fallback preserves the historical behaviour for installations that
        do not yet have the setting.
        """
        return self.bot.config.getboolean(
            'Bot', 'command_cooldowns_enabled', fallback=True
        )
    ...
    def is_channel_allowed(self, message: MeshMessage) -> bool:
        ...
        # An explicitly mentioned LLM request may select one of the configured,
        # read-only tool commands. The local LLM router records the exact selected
        # command so only that command can cross the global channel boundary.
        routing_info = message.routing_info or {}
        mentioned_tool = str(
            routing_info.get("llm_mentioned_tool_command", "")
        ).strip().lower()
        if mentioned_tool and mentioned_tool == self.name.lower():
            return True
    ...
    def build_numbered_chunks(self, lines: List[str], max_chunk_length: int) -> List[str]:
        ...
        def split_line_for_limit(text: str, limit: int) -> List[str]:
            ...
            if limit <= 0:
                return [text]
            if self.get_transport_text_length(text) <= limit:
                return [text]

            pieces: List[str] = []
            remaining = text
            while self.get_transport_text_length(remaining) > limit:
                used_bytes = 0
                hard_split_at = 0
                last_space_at = -1
                for index, character in enumerate(remaining):
                    character_bytes = self.get_transport_text_length(character)
                    if used_bytes + character_bytes > limit:
                        break
                    used_bytes += character_bytes
                    hard_split_at = index + 1
                    if character.isspace():
                        last_space_at = index

                # A normal MeshCore limit is much larger than one UTF-8 code
                # point. Retain a progress fallback for defensive callers.
                if hard_split_at == 0:
                    hard_split_at = 1

                if last_space_at > 0:
                    pieces.append(remaining[:last_space_at])
                    remaining = remaining[last_space_at + 1:]
                else:
                    pieces.append(remaining[:hard_split_at])
                    remaining = remaining[hard_split_at:]
            if remaining:
        ...
        def pack_lines(total_chunks: int) -> List[str]:
            ...
            for raw_line in lines:
                ...
                content_limit = max_chunk_length - self.get_transport_text_length(prefix)
    ...
    def check_cooldown(self, user_id: Optional[str] = None) -> Tuple[bool, float]:
        ...
        """
        if not self.command_cooldowns_enabled() or self.cooldown_seconds <= 0:
            return True, 0.0
