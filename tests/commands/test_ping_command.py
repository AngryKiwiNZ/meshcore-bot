...
class TestPingCommand:
    ...
    def test_can_execute_in_unmonitored_channel_when_enabled(self, command_mock_bot):
        command_mock_bot.config.add_section("Ping_Command")
        command_mock_bot.config.set("Ping_Command", "enabled", "true")
        cmd = PingCommand(command_mock_bot)
        msg = mock_message(content="ping", channel="#nelson", is_dm=False)
        assert cmd.can_execute(msg) is True

    def test_non_ping_stays_blocked_in_unmonitored_channel(self, command_mock_bot):
        command_mock_bot.config.add_section("Ping_Command")
        command_mock_bot.config.set("Ping_Command", "enabled", "true")
        cmd = PingCommand(command_mock_bot)
        msg = mock_message(content="hello", channel="#nelson", is_dm=False)
