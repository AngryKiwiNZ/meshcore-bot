...
class CommandManager:
    ...
    def _should_queue_command(self, command: BaseCommand, message: MeshMessage) -> Tuple[bool, float]:
        ...
        if (
            not command.command_cooldowns_enabled()
            or command.cooldown_seconds <= 0
        ):
            return False, 0.0
    ...
    def _is_channel_trigger_allowed(self, trigger: str, message: MeshMessage) -> bool:
        ...
        if message.is_dm:
            return True
        if self.is_public_channel(message) and trigger.lower() != 'ping':
            return False
        if self.channel_keywords is None:
            return True
        return trigger.lower() in self.channel_keywords

    def is_public_channel(self, message: MeshMessage) -> bool:
        """Return True when the message arrived on the public channel."""
        if message.is_dm or not message.channel:
            return False
        return message.channel.strip().lower().lstrip('#') == 'public'
    ...
    def is_command_trigger(
        self,
        content: str,
        ignored_commands: Optional[set] = None,
        message: Optional[MeshMessage] = None,
    ) -> bool:
        """Return whether text belongs to the normal deterministic command router.

        This is intentionally side-effect free so conversational services can
        preserve command priority without duplicating the command registry.
        Callers may ignore selected novelty commands while retaining precise
        utility-command priority.
        """
        candidate = (content or '').strip()
        if not candidate:
            return False
        if self.command_prefix:
            if not candidate.startswith(self.command_prefix):
                return False
            candidate = candidate[len(self.command_prefix):].strip()
        ...
        # Stateful commands (for example the build/channels submenus) key their
        # active menu by the real sender. Preserve that identity when this check
        # is being made on behalf of an inbound message; a synthetic sender
        # makes valid numeric submenu replies look like ordinary conversation.
        probe = MeshMessage(
            content=(content or '').strip(),
            sender_id=message.sender_id if message else 'llm-router',
            sender_pubkey=message.sender_pubkey if message else None,
            channel=message.channel if message else None,
            is_dm=message.is_dm if message else True,
            timestamp=message.timestamp if message else None,
            routing_info=message.routing_info if message else None,
        )
        for registered_name, command in self.commands.items():
            command_names = {
            ...
            if ignored.intersection(command_names):
                continue
            try:
                if command.should_execute(probe):
                    return True
            except Exception:
                # A malformed optional command must not break all LLM routing.
                continue

        lowered = candidate.lower()
        for keyword in self.keywords:
            trigger = str(keyword).strip().lower()
            if lowered == trigger or lowered.startswith(trigger + ' '):
                return True
        return self.match_randomline(probe) is not None
        ...
        for help_keyword in help_keywords:
            if content_lower.startswith(help_keyword + ' ') or content_lower == help_keyword:
                ...
                if content_lower.startswith(help_keyword + ' '):
                    ...
                    help_messages = self.get_general_help_messages(message)
                    matches.append(('help', help_messages))
                    return matches
    ...
    async def send_dm(
        ...
        try:
            # Prefer the public key carried by the inbound message. Contact names
            # are not unique and an old same-name identity can otherwise receive
            # a reply encrypted for the wrong device.
            contact = None
            if recipient_pubkey and hasattr(
                self.bot.meshcore, "get_contact_by_key_prefix"
            ):
                contact = self.bot.meshcore.get_contact_by_key_prefix(
                    recipient_pubkey
                )
            if not contact:
            ...
            if not contact:
                self.logger.error(
                    "Contact not found for recipient: %s (key: %s)",
                    recipient_id,
                    (recipient_pubkey or "not supplied")[:16],
                )
                return False
    ...
    def get_general_help_messages(self, message: MeshMessage) -> List[str]:
        """Return one or more messages for bare help requests."""
        if 'help' in self.keywords:
            return [self.format_keyword_response(self.keywords['help'], message)]

        help_command = self.commands.get('help')
        if help_command and hasattr(help_command, 'get_general_help_messages'):
            return help_command.get_general_help_messages(message)

        return [self.format_keyword_response(self.get_general_help(message), message)]
        ...
        try:
            ...
            if message.is_dm:
                return await self.send_dm(
                    ...
                    recipient_pubkey=message.sender_pubkey,
                )
            else:
