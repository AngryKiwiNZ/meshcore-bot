...
class HelpCommand(BaseCommand):
    ...
    def get_general_help_messages(self, message: MeshMessage) -> list[str]:
        """Build a practical, LoRa-sized quick guide for common commands."""
        lines = [
            "Quick help - common commands:",
            "ping - Check the bot is reachable; replies Pong.",
            "cmd - Show the compact list of available commands.",
            "advert - DM only: ask the bot radio to send a flood advert (1-hour cooldown).",
            "sun - Show local sunrise and sunset times.",
            "airplane or airplanes [location] [radius=N] - Show aircraft overhead; both names work.",
            "stats [messages|channels|paths|adverts] - Show activity from the past 24 hours.",
            "satpass <NORAD|shortcut> [visual] - Predict passes, e.g. satpass iss.",
            "More: help <command>. Repeater setup: build.",
        ]
        return self.build_numbered_chunks(lines, self.get_numbered_chunk_max_length(message))
