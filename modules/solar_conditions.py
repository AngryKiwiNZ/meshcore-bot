...
def _get_satpass_display_times(pass_time: int, pass_duration: int):
    """Convert pass timestamps to the configured display timezone."""
    ...
    if _config is not None:
        try:
            tz, _ = get_config_timezone(_config, logger)
            return pass_utc.astimezone(tz), set_utc.astimezone(tz)
        except Exception as e:
            logger.debug(f"Falling back to system timezone for satellite pass display: {e}")

    return pass_utc.astimezone(), set_utc.astimezone()
        ...
        try:
            ...
            if next_pass_data.ok:
                ...
                if 'info' in pass_json and passes_count > 0:
                    ...
                    # Convert UTC timestamps to the configured display timezone
                    pass_local, set_local = _get_satpass_display_times(pass_time, pass_duration)
