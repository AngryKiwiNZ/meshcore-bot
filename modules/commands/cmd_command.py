...
class CmdCommand(BaseCommand):
    ...
    def _is_command_valid_for_channel(self, cmd_name: str, cmd_instance: Any, message: Optional[MeshMessage]) -> bool:
        ...
        if (
            not message.is_dm
            and bool(getattr(cmd_instance, 'requires_dm', False))
        ):
            return False
    ...
    def _get_commands_list(self, message: Optional[MeshMessage] = None, max_length: Optional[int] = None) -> str:
        ...
        priority_commands = [
            'test', 'ping', 'help', 'cmd', 'games', 'build', 'advert',
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            ...
            if len(parts) >= 2 and parts[0] in self.keywords:
                if parts[1] in {'game', 'games'}:
                    return await self.send_response(
                        message,
                        "Games (DM): lemonade, blackjack, mastermind. "
                        "Send games for instructions.",
                    )
