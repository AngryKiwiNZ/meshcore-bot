...
class TestCommandTriggerDetection:
    def test_configured_keyword_is_detected_without_executing(self, cm_bot):
        manager = make_manager(cm_bot)
        assert manager.is_command_trigger("ping") is True
        assert manager.is_command_trigger("ping extra") is True
        assert manager.is_command_trigger("ordinary conversation") is False

    def test_command_prefix_is_respected(self, cm_bot):
        cm_bot.config.set("Bot", "command_prefix", "!")
        manager = make_manager(cm_bot)
        assert manager.is_command_trigger("ping") is False
        assert manager.is_command_trigger("!ping") is True

    def test_selected_novelty_command_can_be_ignored(self, cm_bot):
        hello = Mock()
        hello.name = "hello"
        hello.should_execute.return_value = True
        manager = make_manager(cm_bot, {"hello": hello})

        assert manager.is_command_trigger("Hi") is True
        assert manager.is_command_trigger("Hi", ignored_commands={"hello"}) is False


@pytest.mark.asyncio
async def test_dm_response_prefers_inbound_public_key(cm_bot):
    manager = make_manager(cm_bot)
    manager.send_dm = AsyncMock(return_value=True)
    message = MeshMessage(
        content="ping",
        sender_id="Duplicate Name",
        sender_pubkey="ab" * 32,
        is_dm=True,
    )

    assert await manager.send_response(message, "Pong!") is True
    manager.send_dm.assert_awaited_once_with(
        "Duplicate Name",
        "Pong!",
        skip_user_rate_limit=False,
        rate_limit_key="ab" * 32,
...
class TestCheckKeywords:
    ...
    def test_bare_help_returns_multipart_quick_guide(self, cm_bot):
        cm_bot.config.remove_option("Keywords", "help")
        help_command = Mock()
        help_command.keywords = ["help"]
        help_command.get_general_help_messages.return_value = ["1/2 ping - Pong", "2/2 cmd - command list"]
        manager = make_manager(cm_bot, commands={"help": help_command})

        matches = manager.check_keywords(mock_message(content="help", is_dm=True))

        assert matches == [("help", ["1/2 ping - Pong", "2/2 cmd - command list"])]
        help_command.get_general_help_messages.assert_called_once()

    def test_specific_help_remains_single_response(self, cm_bot):
        cm_bot.config.remove_option("Keywords", "help")
        help_command = Mock()
        help_command.keywords = ["help"]
        ping_command = Mock()
        ping_command.get_help_text.return_value = "Replies Pong."
        manager = make_manager(cm_bot, commands={"help": help_command, "ping": ping_command})

        matches = manager.check_keywords(mock_message(content="help ping", is_dm=True))

        assert len(matches) == 1
        assert matches[0][0] == "help"
        assert isinstance(matches[0][1], str)
        assert "Replies Pong." in matches[0][1]
