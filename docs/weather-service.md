...
# weather_location = Example City  # Option B: place name
...
If the configured or requested location cannot be resolved, the shared weather settings can fall back to `default_weather_location` when configured.

Optional pinning for a specific NZ forecast page:
```ini
[Weather]
weather_provider = metservice
metservice_location_path = /towns-cities/regions/<region>/locations/<location>
```
