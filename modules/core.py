...
COMPANION_CLOCK_ROLLOVER_EPOCH = 0xFFFFFFFF
...
class MeshCoreBot:
    ...
    def __init__(self, config_file: str = "config.ini"):
        ...
        try:
            ...
            self.logger.info(
                "Game service initialized (enabled=%s, games=%s)",
                self.game_service.enabled,
                ", ".join(self.game_service._enabled_games()),
            )
        except Exception as e:
            self.logger.warning(f"Failed to initialize game service: {e}")
            self.game_service = None
        try:
            self.llm_service = LLMService(self)
            self.logger.info(
                "LLM service initialized (enabled=%s, model=%s)",
                self.llm_service.enabled,
...
# Example: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa,another_key_here
admin_pubkeys = 
...
ping = "Pong!"
pong = "Ping!"
help = "Bot Help: test, ping, help, cmd, checkin, advert, t phrase, @string, wx, aqi, sun, moon, solar, hfcond, satpass | Use 'help <command>' for details"
cmd = "Available commands: test, ping, help, cmd, checkin, advert, t phrase, @string, wx, aqi, sun, moon, solar, hfcond, satpass"
...
"""
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self.meshcore.is_connected:
                self.connected = True
                # The companion radio updates its contact list when adverts are
                # received. Keep meshcore_py's in-memory view synchronized so a
                # newly advertised contact can be replied to without a restart.
                ...
                # Set radio clock if needed. A future companion clock must be
                # moved back and rebooted so its transient unique/replay clock
                # is cleared before it can timestamp remote admin commands.
                clock_ready = await self.set_radio_clock()
                if not clock_ready:
                    if getattr(self, "_radio_clock_reboot_pending", False):
                        self._radio_clock_reboot_pending = False
                        repair_attempts = getattr(
                            self, "_radio_clock_repair_attempts", 0
                        ) + 1
                        self._radio_clock_repair_attempts = repair_attempts
                        if repair_attempts > 2:
                            self.logger.error(
                                "Companion clock remained invalid after two reboot attempts"
                            )
                            return False
                        self.logger.info(
                            "Reconnecting after companion clock correction"
                        )
                        return await self.connect()
                    return False
                self._radio_clock_repair_attempts = 0

                # Keep the companion's route hash width aligned with the local
                # mesh. Existing contacts retain their per-route hash width;
                # this setting governs newly learned/discovered paths.
                await self.ensure_device_path_hash_mode()
    ...
    async def set_radio_clock(self) -> bool:
        """Set the companion clock and safely recover it when it is in the future.
        ...
        try:
            ...
            clock_drift = device_time - current_time
            if clock_drift > 5:
                if getattr(self, "_radio_clock_post_reboot_rollover", False):
                    self.logger.info(
                        "Completing companion clock rollover after replay-state reboot"
                    )
                    repaired = await self._finish_future_radio_clock_repair(
                        current_time
                    )
                    if repaired:
                        self._radio_clock_post_reboot_rollover = False
                        self.last_clock_sync_time = current_time
                    return repaired
                self.logger.warning(
                    "Device clock is %s seconds ahead; correcting and rebooting "
                    "the companion to clear its transient replay timestamp",
                    clock_drift,
                )
                repaired = await self._correct_future_radio_clock_and_reboot(
                    current_time
                )
                if repaired:
                    self._radio_clock_reboot_pending = True
                return False
            ...
            if device_time < current_time:
                ...
                self.logger.info("Device time is current")
                return True
    ...
    async def _correct_future_radio_clock_and_reboot(self, host_epoch: int) -> bool:
        """Roll a future companion RTC over, then reboot to clear unique time.

        Companion firmware deliberately refuses backwards SET_DEVICE_TIME
        requests. Advancing its uint32 clock to the final epoch and rebooting
        lets it wrap to a low value; normal startup can then safely advance it
        to ``host_epoch`` while the transient unique timestamp is clean.
        """
        radio = self.meshcore
        if radio is None:
            return False
        try:
            result = await radio.commands.set_time(
                COMPANION_CLOCK_ROLLOVER_EPOCH
            )
            if result is None or result.type != EventType.OK:
                self.logger.error("Failed to advance companion clock for rollover")
                return False
            verification = await radio.commands.get_time()
            rollover_epoch = int(verification.payload.get("time", 0))
            if not (
                rollover_epoch >= COMPANION_CLOCK_ROLLOVER_EPOCH - 2
                or rollover_epoch <= 5
            ):
                self.logger.error(
                    "Companion clock rollover did not verify (epoch=%s)",
                    rollover_epoch,
                )
                return False

            self.logger.info(
                "Companion RTC advanced to rollover epoch; rebooting to clear replay state"
            )
            try:
                await radio.commands.reboot()
            finally:
                try:
                    ...
                    pass
            self.meshcore = None
            self._radio_clock_post_reboot_rollover = True
            await asyncio.sleep(8)
            return True
        except Exception as exc:
            self.logger.error("Future companion clock repair failed: %s", exc)
            return False

    async def _finish_future_radio_clock_repair(self, host_epoch: int) -> bool:
        """Wrap the rebooted companion clock, then advance it to host UTC."""
        radio = self.meshcore
        if radio is None:
            return False
        try:
            rollover = await radio.commands.set_time(
                COMPANION_CLOCK_ROLLOVER_EPOCH
            )
            if rollover is None or rollover.type != EventType.OK:
                self.logger.error("Failed to start post-reboot companion rollover")
                return False
            # The protocol clock is uint32. Give it time to cross from
            # 0xFFFFFFFF to a low epoch so a normal SET_DEVICE_TIME is forward.
            await asyncio.sleep(2)
            result = await radio.commands.set_time(host_epoch)
            if result is None or result.type != EventType.OK:
                self.logger.error("Failed to set companion clock after rollover")
                return False
            verification = await radio.commands.get_time()
            corrected_epoch = int(verification.payload.get("time", 0))
            corrected_drift = corrected_epoch - int(time.time())
            if abs(corrected_drift) > 5:
                self.logger.error(
                    "Post-rollover companion time did not verify (drift=%ss)",
                    corrected_drift,
                )
                return False
            self.logger.info(
                "Companion clock recovered and verified (drift=%ss)",
                corrected_drift,
            )
            return True
        except Exception as exc:
            self.logger.error("Post-reboot companion rollover failed: %s", exc)
            return False

    async def repair_future_radio_clock(self) -> bool:
        """Repair a future companion clock at runtime and rebuild its connection."""
        if self.meshcore is None:
            return False
        try:
            time_result = await self.meshcore.commands.get_time()
            device_epoch = int(time_result.payload.get("time", 0))
            host_epoch = int(time.time())
            if device_epoch - host_epoch <= 5:
                return True
            if not await self._correct_future_radio_clock_and_reboot(host_epoch):
                return False
            return await self.connect()
        except Exception as exc:
            self.logger.error("Runtime companion clock repair failed: %s", exc)
            return False
    ...
    async def ensure_device_path_hash_mode(self) -> bool:
        """Verify and, when configured, enforce the companion path hash mode.

        MeshCore represents one-, two-, and three-byte hop hashes as modes 0,
        1, and 2 respectively. The setting is optional for backwards
        compatibility; a blank value leaves the companion unchanged.
        """
        raw_mode = self.config.get("Bot", "path_hash_mode", fallback="").strip()
        if not raw_mode:
            return True
        try:
            desired_mode = int(raw_mode)
        except ValueError:
            self.logger.warning("Ignoring invalid Bot.path_hash_mode %r", raw_mode)
            return False
        if desired_mode not in (0, 1, 2):
            self.logger.warning(
                "Ignoring unsupported Bot.path_hash_mode %s; expected 0, 1, or 2",
                desired_mode,
            )
            return False
        if not self.meshcore or not self.meshcore.is_connected:
            self.logger.warning("Cannot check path hash mode - not connected to device")
            return False

        try:
            current_mode = await self.meshcore.commands.get_path_hash_mode()
            if current_mode == desired_mode:
                self.logger.info(
                    "Device path hash mode already %s (%s-byte routing)",
                    desired_mode,
                    desired_mode + 1,
                )
                return True
            ...
            self.logger.info(
                "Updating device path hash mode from %s to %s (%s-byte routing)",
                current_mode,
                desired_mode,
                desired_mode + 1,
            )
            result = await self.meshcore.commands.set_path_hash_mode(desired_mode)
            if result is None or result.type != EventType.OK:
                self.logger.warning("Failed to update device path hash mode")
                return False
            verified_mode = await self.meshcore.commands.get_path_hash_mode()
            if verified_mode != desired_mode:
                self.logger.warning(
                    "Device path hash mode did not verify (expected %s, got %s)",
                    desired_mode,
                    verified_mode,
                )
                return False
            self.logger.info(
                "Device path hash mode verified at %s-byte routing",
                desired_mode + 1,
            )
            return True
        except (OSError, AttributeError, TypeError, ValueError) as exc:
            self.logger.warning("Error checking/setting device path hash mode: %s", exc)
            return False
    ...
    async def start(self) -> None:
        ...
        # A dedicated LLM host can keep its model resident, avoiding the large
        # cold-start delay before the first conversational reply.
        if self.llm_service:
