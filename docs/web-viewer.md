...
1. Edit `config.ini` and set:
   ...
   host = 127.0.0.1
   port = 5000
   # Set a unique password in your local config.ini.
   access_password =
   ```
...
# Optional password to unlock write access in the web UI
# Set a unique password in your local config.ini.
access_password =
```
...
If `access_password` is set, the viewer stays publicly readable. Administrative
pages such as Cache, LLM, and Games, plus most write actions, require logging in.
The individual repeater `Poll Now` and `Clock Sync` buttons remain public.

## Repeater Monitor Discovery

The repeater monitor can combine:
- manual targets from `data/repeater_monitor_nodes.txt`
- auto-discovered repeaters from the Contacts database

Useful `[Repeater_Monitor]` settings:

```ini
[Repeater_Monitor]
enabled = true
nodes_file = data/repeater_monitor_nodes.txt
auto_discover_contacts = true
auto_discovery_radius_km = 50
auto_discovery_latitude = <latitude>
auto_discovery_longitude = <longitude>
```

The override file format is:

```text
public_key_or_unique_prefix,Display Name[,enabled]
```

Examples:
- `ce1dbff8202a,B2 - RAK4631 Repeater`
- `ce1dbff8202a,B2 - RAK4631 Repeater,disabled`

If a repeater is auto-discovered from Contacts but you do not want it polled, add it to the file with `disabled` as the third field.

Each repeater detail page includes:

- `Poll Now`, which queues an immediate telemetry check
- `Clock Sync`, which logs in and sends `clkreboot` followed by `clock sync`
- the latest 25 command-log entries, including attempts and timeouts
- battery, advertised-clock drift, and published-temperature history

Both action buttons are available to public viewers. For `Clock Sync`, add the
bot's MeshCore identity to the repeater administrator list; no password is stored
in the web viewer.
...
- The `/games` page and all `/api/games/` endpoints require the password-protected
  session. They are not exposed to public read-only repeater viewers.
- The Games page controls individual game availability, active DM sessions, and
  persistent leaderboards.
- Set `host = 127.0.0.1` for localhost-only access
- Set `host = 0.0.0.0` for network access (use with caution)
- Set `access_password` when exposing the viewer beyond localhost
- Firewall rules are still recommended for production use
