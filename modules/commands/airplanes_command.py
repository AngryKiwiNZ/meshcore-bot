...
class AirplanesCommand(BaseCommand):
    ...
    # Plugin metadata
    name = "airplanes"
    keywords = ['airplane', 'airplanes', 'aircraft', 'planes', 'adsb', 'overhead']
    ...
    usage = "airplanes [place|lat,lon|here] [radius|radius=N] [options]"
    examples = ["airplanes Example City", "airplanes Example City 100nm", "overhead 47.6,-122.3"]
    parameters = [
        {"name": "location", "description": "Place name, coordinates, or here for the bot location"},
    ...
    def __init__(self, bot):
        ...
        self.api_url = self.get_config_value('Airplanes_Command', 'api_url', fallback='https://api.airplanes.live/v2/', value_type='str')
        ...
        self.default_location_name = (
            ...
            or self.bot.config.get('Weather', 'default_weather_location', fallback='').strip()
        )
        
        # Ensure API URL ends with /
        if self.api_url and not self.api_url.endswith('/'):
            self.api_url += '/'
        if self.api_url.startswith('http://api.airplanes.live/'):
            self.api_url = 'https://' + self.api_url[len('http://'):]
    ...
    def _resolve_named_location(self, location_name: str) -> Optional[Tuple[float, float]]:
        """Resolve an explicitly requested place name for an aircraft lookup."""
        try:
            lat, lon, _ = geocode_city_sync(
                self.bot,
                location_name,
            ...
            self.logger.debug(f"Error geocoding airplanes location '{location_name}': {e}")
            return None

    def _split_location_and_filters(self, args: List[str]) -> Tuple[str, List[str]]:
        """Separate a free-text location from filters and radius shorthand."""
        location_parts = []
        filter_args = []
        filter_prefixes = (
            'radius=', 'alt=', 'speed=', 'type=', 'callsign=', 'squawk=', 'limit='
        )
        filter_flags = {
            'military', 'ladd', 'pia', 'closest', 'distance', 'highest', 'altitude', 'fastest'
        }
        radius_pattern = re.compile(r'^(\d+(?:\.\d+)?)(?:nm)?$', re.IGNORECASE)

        for arg in args:
            arg_lower = arg.lower()
            radius_match = radius_pattern.fullmatch(arg_lower)
            if radius_match:
                filter_args.append(f"radius={radius_match.group(1)}")
            elif arg_lower.startswith(filter_prefixes) or arg_lower in filter_flags:
                filter_args.append(arg)
            else:
                location_parts.append(arg)

        location_name = ' '.join(location_parts).strip().strip(',').strip()
        return location_name, filter_args
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            ...
            if is_overhead_command:
                ...
                # Resolve free-text place names and accept radius shorthand such as
                # "100", "100nm", or a trailing value in "Example City 50".
                location_name, args = self._split_location_and_filters(args)
                if location is None and location_name:
                    location = self._resolve_named_location(location_name)
                    if location:
                        location_source = "named"
                    else:
                        await self.send_response(message, f"Location not found: {location_name}")
                        return True
