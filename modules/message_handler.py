...
class MessageHandler:
    ...
    async def handle_contact_message(self, event, metadata=None):
        ...
        try:
            ...
            sender_is_infrastructure = False
            if hasattr(self.bot.meshcore, 'contacts') and self.bot.meshcore.contacts:
                for contact_key, contact_data in self.bot.meshcore.contacts.items():
                    if contact_data.get('public_key', '').startswith(sender_id):
                        ...
                        # Repeater and room-server DMs are remote CLI responses,
                        # not human conversation.  Feeding them to the LLM can
                        # cause an unsolicited reply that competes with the next
                        # administration command on the radio.
                        sender_is_infrastructure = contact_data.get('type') in (2, 3)
                        break
            ...
            if sender_is_infrastructure:
                self.logger.debug(
                    "Ignoring repeater/room-server CLI response from %s in the conversation handler",
                    sender_name,
                )
                return
    ...
    async def process_message(self, message: MeshMessage):
        ...
        if ('greeter' in self.bot.command_manager.commands and
                not self.bot.command_manager.is_public_channel(message)):
        ...
        # Mentioned channel LLM replies are intentionally available on any channel.
        # Natural-language requests may be rewritten to an approved read-only Python
        # command. LLMService marks that narrow route so it can pass the general
        # monitored-channel filter without opening unrelated commands.
        llm_service = getattr(self.bot, "llm_service", None)
        if not message.is_dm and llm_service and await llm_service.handle_message(message):
            self.logger.info("Queued mentioned channel LLM reply for %s", message.channel)
            return
        ...
        # Active DM games and game commands take precedence over conversational
        # LLM routing. GameService explicitly yields known utility commands so
        # wx/ping/etc. remain available without ending a game.
        game_service = getattr(self.bot, "game_service", None)
        if message.is_dm and game_service and await game_service.handle_message(message):
            self.logger.info("Handled DM game message for %s", message.sender_id)
            return

        # Non-command Bot-radio DMs are conversational by default. The LLM router
        # returns False for deterministic menu/Python commands so they retain
        # priority in the normal command path below.
        if llm_service and await llm_service.handle_message(message):
            self.logger.info("Queued conversational LLM reply for %s", message.sender_id)
            return
        ...
        if keyword_matches:
            for keyword, response in keyword_matches:
                ...
                # Send response (bare help may be a paced multipart guide)
                try:
                    ...
                    if isinstance(response, list):
                        success = await self.bot.command_manager.send_response_chunked(
                            message, response, skip_user_rate_limit_first=True
                        )
                    elif message.is_dm:
                ...
                if (hasattr(self.bot, 'web_viewer_integration') and 
                    ...
                    try:
                        ...
                        self.bot.web_viewer_integration.bot_integration.capture_command(
                            message, keyword, captured_response, success, command_id
                        )
                    except Exception as e:
    ...
    def should_process_message(self, message: MeshMessage) -> bool:
        ...
        routing_info = message.routing_info or {}
        mentioned_tool = str(
            routing_info.get("llm_mentioned_tool_command", "")
        ).strip().lower()
        llm_service = getattr(self.bot, "llm_service", None)
        approved_tools = getattr(llm_service, "tool_commands", set()) if llm_service else set()
        if (
            not message.is_dm
            and mentioned_tool in approved_tools
            and mentioned_tool in self.bot.command_manager.commands
        ):
            self.logger.debug(
                "Allowing mentioned LLM tool command '%s' on channel %s",
                mentioned_tool,
                message.channel,
            )
            return True
