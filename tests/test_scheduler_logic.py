...
class TestScheduledCommands:
    @pytest.mark.asyncio
    async def test_executes_loaded_command_for_target_channel(self, scheduler):
        command = Mock()
        command.execute = AsyncMock(return_value=True)
        scheduler.bot.command_manager.commands = {"wx": command}

        await scheduler._send_scheduled_message_async("#nsnweather", "command:wx nelson")

        command.execute.assert_awaited_once()
        message = command.execute.await_args.args[0]
        assert message.content == "wx nelson"
        assert message.channel == "#nsnweather"
        assert message.sender_id == "scheduler:#nsnweather"
        assert message.is_dm is False

    @pytest.mark.asyncio
    async def test_static_scheduled_message_remains_supported(self, scheduler):
        scheduler.bot.command_manager.send_channel_message = AsyncMock(return_value=True)

        await scheduler._send_scheduled_message_async("#general", "Good morning")
