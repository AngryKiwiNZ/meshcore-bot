...
class GlobalWxCommand(BaseCommand):
    ...
    def __init__(self, bot: Any):
        ...
        self.default_weather_location = self.bot.config.get(
            'Weather', 'default_weather_location', fallback=''
        ).strip()
        self.use_companion_location_for_default = self.bot.config.getboolean(
            'Weather', 'use_companion_location_for_default', fallback=True
        )
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        if len(parts) < 2:
            ...
            companion_location = (
                self._get_companion_location(message)
                if self.use_companion_location_for_default
                else None
            )
            if companion_location:
                ...
                if self.default_weather_location:
                    parts = [parts[0], self.default_weather_location]
                    self.logger.info("No companion location found; using configured weather default")
        ...
        if not location and self.default_weather_location:
            location = self.default_weather_location
            self.logger.info("No location provided after parsing; using configured weather default")

        if not location:
            await self.send_response(message, self.translate('commands.gwx.no_location', location='a location'))
            return True
    ...
    async def get_weather_for_location(self, location: str, forecast_type: str = "default", num_days: int = 7, message: MeshMessage = None) -> Union[str, Tuple[str, List[str]]]:
        ...
        try:
            ...
            if not result or result[0] is None or result[1] is None:
                ...
                default_location = self.default_weather_location.strip()
                if default_location and normalized_location and normalized_location != default_location.lower():
                    self.logger.info(
                        f"Could not resolve weather location '{location}'; retrying with default location '{default_location}'"
                    )
                    result = self.geocode_location(default_location)
            ...
            if using_metservice and metservice_weather:
                self.logger.info(
                    "Using MetService weather for %s via %s",
                    location_display,
                    metservice_path,
                )
                ...
                self.logger.info(
                    "MetService weather unavailable for %s; using Open-Meteo fallback",
                    location_display,
                )
