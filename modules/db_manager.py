...
def ensure_llm_schema(conn: sqlite3.Connection) -> None:
    """Create/migrate the LLM tables from one shared definition."""
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS llm_contacts (
            public_key TEXT PRIMARY KEY,
            contact_name TEXT NOT NULL,
            enabled INTEGER NOT NULL DEFAULT 0,
            interaction_mode TEXT NOT NULL DEFAULT 'automatic',
            profile_scope TEXT NOT NULL DEFAULT 'dm',
            radio_id TEXT NOT NULL DEFAULT 'bot',
            profile_file TEXT,
    ...
    ''')
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS llm_dm_optins (
            public_key TEXT PRIMARY KEY,
            enabled INTEGER NOT NULL DEFAULT 0,
            updated_at REAL NOT NULL DEFAULT (unixepoch())
        )
    ''')
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_llm_messages_contact
        ON llm_messages(public_key, id DESC)
    ''')
    for statement in (
        "ALTER TABLE llm_contacts ADD COLUMN interaction_mode TEXT NOT NULL DEFAULT 'automatic'",
        "ALTER TABLE llm_contacts ADD COLUMN profile_scope TEXT NOT NULL DEFAULT 'dm'",
        "ALTER TABLE llm_messages ADD COLUMN message_timestamp REAL",
    ):
        try:
...
def ensure_game_schema(conn: sqlite3.Connection) -> None:
    """Create game tables without importing the runtime game service."""
    from .game_service import ensure_game_schema as create_game_schema

    create_game_schema(conn)
...
class DBManager:
    ...
    ALLOWED_TABLES = {
        ...
        'checkins',  # Check-in / roll-call command history
        'repeater_monitor_nodes',
        'repeater_monitor_samples',
        'llm_contacts',
        'llm_messages',
        'llm_settings',
        'llm_dm_optins',
            ...
            with self.connection() as conn:
                ...
                cursor.execute('''
                    CREATE TABLE IF NOT EXISTS feed_subscriptions (
                        ...
                        chunking_enabled BOOLEAN DEFAULT 0,
                ...
                try:
                    ...
                    pass  # Column already exists
                try:
                    cursor.execute('ALTER TABLE feed_subscriptions ADD COLUMN chunking_enabled BOOLEAN DEFAULT 0')
                except sqlite3.OperationalError:
                    pass  # Column already exists
                ...
                ''')
                try:
                    cursor.execute('ALTER TABLE feed_message_queue ADD COLUMN chunk_index INTEGER DEFAULT 0')
                except sqlite3.OperationalError:
                    pass  # Column already exists
                try:
                    cursor.execute('ALTER TABLE feed_message_queue ADD COLUMN chunk_count INTEGER DEFAULT 1')
                except sqlite3.OperationalError:
                    pass  # Column already exists
                try:
                    cursor.execute('ALTER TABLE feed_message_queue ADD COLUMN attempt_count INTEGER DEFAULT 0')
                except sqlite3.OperationalError:
                    pass  # Column already exists
                try:
                    cursor.execute('ALTER TABLE feed_message_queue ADD COLUMN last_attempt_at TIMESTAMP')
                except sqlite3.OperationalError:
                    pass  # Column already exists
