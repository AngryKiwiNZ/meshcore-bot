...
def split_message(message: str, max_length: int) -> List[str]:
    """Split a message at line/word boundaries within a UTF-8 byte limit."""
    def transport_length(text: str) -> int:
        return len(text.encode('utf-8'))

    def fitting_prefix_length(text: str) -> int:
        used = 0
    ...
    if not message or max_length <= 0:
        return []
    if transport_length(message) <= max_length:
        return [message]
    ...
    for original_line in message.split('\n'):
        line = original_line
        while transport_length(line) > max_length:
            flush_current()
            prefix_length = fitting_prefix_length(line)
            break_at = line.rfind(' ', 0, prefix_length + 1)
            if break_at <= 0:
                break_at = prefix_length
            chunks.append(line[:break_at])
            line = line[break_at:].lstrip()

        if not line:
            if current and transport_length(current) < max_length:
                current += '\n'
            continue

        candidate = line if not current else f"{current}\n{line}"
        if transport_length(candidate) <= max_length:
            current = candidate
        else:
            flush_current()
            current = line
...
def extract_cap_areas(rss_content: str) -> Dict[str, str]:
    """Return CAP area descriptions keyed by the parent RSS item GUID."""
    areas_by_id: Dict[str, str] = {}
    try:
    ...
    for item in root.findall('./channel/item'):
        item_id = (item.findtext('guid') or item.findtext('link') or '').strip()
        encoded_cap = item.findtext(encoded_tag)
        if not item_id or not encoded_cap:
            continue
        try:
            cap_root = ET.fromstring(encoded_cap)
        except ET.ParseError:
            continue
        area_names = []
        for element in cap_root.findall('.//{*}areaDesc'):
            area = ' '.join((element.text or '').split())
            if area and area not in area_names:
                area_names.append(area)
        if area_names:
            areas_by_id[item_id] = '; '.join(area_names)
    return areas_by_id


def extract_cap_metadata(cap_content: str) -> Dict[str, str]:
    """Extract safety-relevant fields from a standalone CAP alert document."""
    try:
...
class FeedManager:
    ...
    def __init__(self, bot):
        ...
        if not bot.config.has_section('Feed_Manager'):
            ...
            self.emergency_repeat_wait_seconds = 5.0
            self.emergency_retry_delay_seconds = 10.0
        else:
            ...
            self.emergency_repeat_wait_seconds = bot.config.getfloat(
                'Feed_Manager', 'emergency_repeat_wait_seconds', fallback=5.0
            )
            self.emergency_retry_delay_seconds = bot.config.getfloat(
                'Feed_Manager', 'emergency_retry_delay_seconds', fallback=10.0
            )
        
        # Rate limiting per domain
        self._domain_last_request: Dict[str, float] = {}
        self._cap_metadata_cache: Dict[str, Dict[str, str]] = {}
    ...
    async def process_rss_feed(self, feed: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cap_areas = extract_cap_areas(content)
            ...
            for entry in parsed.entries:
                ...
                # MetService's RSS index links to standalone CAP documents.
                # Fetch each immutable CAP document once so location and
                # severity are available to filters and output formatting.
                entry_link = entry.get('link', '')
                if not raw_data['area'] and 'alerts.metservice.com' in feed_url and entry_link:
                    metadata = self._cap_metadata_cache.get(entry_link)
                    if metadata is None:
                        try:
                            async with self.session.get(entry_link, timeout=timeout) as cap_response:
                                if cap_response.status == 200:
                                    metadata = extract_cap_metadata(await cap_response.text())
                                else:
                                    metadata = {}
                                    self.logger.warning(
                                        "MetService CAP document returned HTTP %s: %s",
                                        cap_response.status, entry_link
                                    )
                        except Exception as e:
                            metadata = {}
                            self.logger.warning("Unable to fetch MetService CAP document %s: %s", entry_link, e)
                        self._cap_metadata_cache[entry_link] = metadata
    ...
    def format_message(self, item: Dict[str, Any], feed: Dict[str, Any]) -> str:
        ...
        # Chunk-enabled feeds are split after formatting; other feeds retain
        # the legacy single-message truncation behaviour.
        if not feed.get('chunking_enabled') and len(message) > self.max_message_length:
    ...
    def _queue_feed_message(self, feed: Dict[str, Any], item: Dict[str, Any], message: str,
        ...
        try:
            priority = 100 if self._is_emergency_feed(feed) else 0
            with self.bot.db_manager.connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO feed_message_queue 
                    (feed_id, channel_name, message, item_id, item_title, priority, chunk_index, chunk_count)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    feed['id'],
                    feed['channel_name'],
    ...
    @staticmethod
    def _is_emergency_feed(feed: Dict[str, Any]) -> bool:
        """Return whether a feed should receive emergency delivery handling."""
        channel = str(feed.get('channel_name', '')).strip().lower().lstrip('#')
        name = str(feed.get('feed_name', '')).strip().lower()
        return channel == 'emergency' or 'emergency' in name

    def _repeat_count(self, command_id: str) -> Optional[int]:
        """Return observed repeat count, or None when tracking is unavailable."""
        tracker = getattr(self.bot, 'transmission_tracker', None)
        if tracker is None or not hasattr(tracker, 'get_repeat_info'):
            return None
        try:
            return int(tracker.get_repeat_info(command_id=command_id).get('repeat_count', 0))
        except Exception as exc:
            self.logger.debug("Unable to inspect repeat confirmation: %s", exc)
            return None

    async def _wait_for_repeat(self, command_id: str, timeout: float) -> Optional[bool]:
        """Wait for a repeater echo; return None if tracking is unavailable."""
        count = self._repeat_count(command_id)
        if count is None:
            return None
    ...
    def _record_queue_attempt(self, queue_id: int) -> int:
        """Persist an attempt before transmitting so retries stay bounded across restarts."""
        with self.bot.db_manager.connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
            ...
            ''', (queue_id,))
            cursor.execute(
                'SELECT attempt_count FROM feed_message_queue WHERE id = ?',
                (queue_id,),
            )
    ...
    async def _send_feed_item(self, feed: Dict[str, Any], item: Dict[str, Any]):
        ...
        try:
            ...
            messages = split_message(message, self.max_message_length) if feed.get('chunking_enabled') else [message]
            for chunk_index, chunk in enumerate(messages):
                self._queue_feed_message(feed, item, chunk, chunk_index, len(messages))
        except Exception as e:
    ...
    async def process_message_queue(self):
        ...
        try:
            ...
            with self.bot.db_manager.connection() as conn:
                ...
                cursor.execute('''
                    SELECT q.id, q.feed_id, q.channel_name, q.message, q.item_id, q.item_title,
                           q.chunk_index, q.chunk_count, q.attempt_count,
                           f.message_send_interval_seconds, f.feed_name
                    FROM feed_message_queue q
                    JOIN feed_subscriptions f ON q.feed_id = f.id
                    WHERE q.sent_at IS NULL
                      AND (
                          NOT (
                              lower(ltrim(q.channel_name, '#')) = 'emergency'
                              OR lower(coalesce(f.feed_name, '')) LIKE '%emergency%'
                          )
                          OR q.attempt_count < 2
                      )
            ...
            for msg in messages:
                ...
                item_key = (feed_id, item_id)
                emergency = self._is_emergency_feed(dict(msg))

                if item_key in blocked_items:
                    continue
                ...
                # Send the message
                try:
                    attempt_count = self._record_queue_attempt(queue_id) if emergency else 0
                    command_id = f"feed_{feed_id}_{queue_id}_{attempt_count or 1}"
                    success = await self.bot.command_manager.send_channel_message(
                        channel_name, message_text, command_id=command_id
                    )

                    if success and emergency and attempt_count < 2:
                        repeated = await self._wait_for_repeat(
                            command_id, self.emergency_repeat_wait_seconds
                        )
                        if repeated is False:
                            await asyncio.sleep(self.emergency_retry_delay_seconds)
                            # Avoid a duplicate if an echo arrived during the retry delay.
                            if self._repeat_count(command_id) == 0:
                                attempt_count = self._record_queue_attempt(queue_id)
                                retry_command_id = f"feed_{feed_id}_{queue_id}_{attempt_count}"
                                self.logger.warning(
                                    "No repeater echo for emergency feed chunk; retrying once: %s",
                                    item_title[:50],
                                )
                                retry_success = await self.bot.command_manager.send_channel_message(
                                    channel_name, message_text, command_id=retry_command_id
                                )
                                if not retry_success:
                                    self._record_feed_error(
                                        feed_id,
                                        'channel',
                                        f"Emergency retry failed for channel {channel_name}",
                                    )
                        elif repeated is None:
                            self.logger.debug(
                                "Repeat tracking unavailable; emergency retry disabled for queue item %s",
                                queue_id,
                            )
                    ...
                    if success:
                        ...
                        # Record one activity entry only after the final chunk.
                        if msg['chunk_index'] >= msg['chunk_count'] - 1:
                        ...
                        if emergency and attempt_count >= 2:
                            self.logger.error(
                                "Emergency feed queue item %s exhausted its single retry",
                                queue_id,
                            )
                        # Emergency items stop after two total attempts; other feeds retain
