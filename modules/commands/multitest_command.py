...
class MultitestCommand(BaseCommand):
    ...
    def extract_path_from_rf_data(self, rf_data: dict) -> Optional[str]:
        ...
        try:
            ...
            # A zero-hop packet is a successful direct route, not an extraction
            # failure. Only treat it as explicit when path_length is present.
            if routing_info.get('path_length') == 0:
                return 'Direct'
            ...
            # Reuse the message handler's packet decoder when cached routing
            # metadata is incomplete. This also handles current multi-byte paths.
            path_helper = getattr(self.bot.message_handler, '_get_path_from_rf_data', None)
            if callable(path_helper):
                path_string, _, hops = path_helper(rf_data)
                if path_string:
                    return path_string
                if hops == 0:
                    return 'Direct'
