...
from .repeater_monitor_targets import restore_suppressed_target
...
class RepeaterManager:
    ...
    async def track_contact_advertisement(self, advert_data: Dict, signal_info: Dict = None, packet_hash: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if (
                role in {'repeater', 'roomserver'}
                and self.bot.config.getboolean('Repeater_Monitor', 'restore_deleted_on_advert', fallback=True)
            ):
                nodes_file = self.bot.config.get(
                    'Repeater_Monitor', 'nodes_file', fallback='data/repeater_monitor_nodes.txt'
                )
                nodes_path = Path(nodes_file)
                if not nodes_path.is_absolute():
                    nodes_path = Path(self.bot.bot_root) / nodes_path
                restored = restore_suppressed_target(str(nodes_path), public_key, name)
                if restored:
                    self.logger.info(
                        "Restored deleted repeater monitor target after advert: %s (%s)",
                        restored['display_name'], restored['node_key'],
                    )
