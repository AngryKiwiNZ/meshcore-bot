...
class BotIntegration:
    ...
    def _internal_stream_headers(self):
        secret = self.bot.config.get(
            'Web_Viewer', 'session_secret', fallback='meshcore_bot_viewer_secret'
        )
        token = hashlib.sha256(
            f'{secret}\0meshcore-stream'.encode('utf-8')
        ).hexdigest()
        return {'X-MeshCore-Internal-Token': token}
            ...
            if self.http_session:
                try:
                    response = self.http_session.post(
                        url, json=payload, headers=self._internal_stream_headers(), timeout=1.0
                    )
    ...
    def send_mesh_node_update(self, node_data):
        ...
        try:
            ...
            try:
                response = requests.post(
                    url, json=payload, headers=self._internal_stream_headers(), timeout=0.5
                )
                response.raise_for_status()
                self._record_web_viewer_result(True)
            except Exception:
...
class WebViewerIntegration:
    ...
    def _validate_config(self):
        ...
        if self.host == '0.0.0.0':
            password_set = self.bot.config.get('Web_Viewer', 'access_password', fallback='').strip()
            self.logger.warning(
                "\n" + "="*70 + "\n"
                ...
                f"{'WITH a login password configured.' if password_set else 'WITHOUT a login password configured.'} "
                "Ensure you have firewall protection!\n"
