...
class MessageScheduler:
    ...
    async def _send_scheduled_message_async(self, channel: str, message: str):
        ...
        if message.lower().startswith('command:'):
            command_text = message.split(':', 1)[1].strip()
            if not command_text:
                self.logger.warning("Scheduled command for %s is empty", channel)
                return
            command_name = command_text.split()[0].lower()
            command = self.bot.command_manager.commands.get(command_name)
            if not command:
                self.logger.error("Scheduled command '%s' is not loaded", command_name)
                return
            scheduled_message = MeshMessage(
                content=command_text,
                sender_id=f"scheduler:{channel}",
                channel=channel,
                is_dm=False,
                timestamp=int(time.time()),
            )
            self.logger.info("Executing scheduled command '%s' for %s", command_text, channel)
            await command.execute(scheduled_message)
            return
