...
class TestHelpCommand:
    ...
    def test_general_help_messages_are_practical_and_lora_sized(self, command_mock_bot):
        command_mock_bot.config.add_section("Help_Command")
        command_mock_bot.config.set("Help_Command", "enabled", "true")
        cmd = HelpCommand(command_mock_bot)

        messages = cmd.get_general_help_messages(mock_message(content="help", is_dm=True))
        combined = "\n".join(messages)

        assert len(messages) > 1
        assert all(len(item.encode("utf-8")) <= 125 for item in messages)
        for command in ("ping", "cmd", "advert", "sun", "airplane", "airplanes", "stats", "satpass"):
            assert command in combined
        assert "help <command>" in combined
