...
class TestIsChannelAllowed:
    ...
    def test_matching_llm_mentioned_tool_is_allowed_on_new_channel(self, command_mock_bot):
        cmd = PingCommand(command_mock_bot)
        msg = mock_message(
            channel="#new-channel",
            is_dm=False,
            routing_info={"llm_mentioned_tool_command": "ping"},
        )
        assert cmd.is_channel_allowed(msg) is True

    def test_llm_tool_marker_does_not_open_another_command(self, command_mock_bot):
        cmd = _TestCommand(command_mock_bot)
        msg = mock_message(
            channel="#new-channel",
            is_dm=False,
...
class TestCanExecute:
    ...
    def test_master_switch_disables_command_cooldown(self, command_mock_bot):
        command_mock_bot.config.set(
            "Bot", "command_cooldowns_enabled", "false"
        )
        cmd = _TestCommand(command_mock_bot)
        cmd.cooldown_seconds = 60
        cmd.record_execution("TestUser")

        assert cmd.check_cooldown("TestUser") == (True, 0.0)
        assert cmd.get_remaining_cooldown("TestUser") == 0
        assert cmd.can_execute(mock_message(is_dm=True)) is True

    def test_command_cooldowns_remain_enabled_by_default(self, command_mock_bot):
        cmd = _TestCommand(command_mock_bot)
        cmd.cooldown_seconds = 60
        cmd.record_execution("TestUser")

        can_execute, remaining = cmd.check_cooldown("TestUser")
